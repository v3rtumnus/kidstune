...
| **Kids App: Animations** ✅ | Shared element transition (MiniPlayerBar thumbnail → NowPlayingScreen cover art via `SharedTransitionLayout`), button press scale (0.95×) + haptic feedback on `ContentTile` and `CategoryButton`, spring-fling pager on `BrowseScreen`/`AlbumGridScreen`, shimmer loading placeholder (`ShimmerBox`), `FavoriteButton` bounce-in/shrink-out via `Animatable`, full-screen confetti `Canvas` on celebration, NEU badge pulse via `InfiniteTransition`, slide-up/fade screen transitions on `NowPlayingRoute` |
